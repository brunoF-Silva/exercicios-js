...
Porém, outras funções que fuja de tal padão estára disponível apenas em um contexto de escopo global, mas não no objeto global window.
    {
        ...
        IMPORTANTE: constantes e variáveis declaradas com let e com var não vão diretamente para dentro do objeto windows, eles ficam em um lugar em meméoria dentro do browser que não conseguimos acessar, porém essas constantes e variáveis, dentro de tal objeto protegido, ainda possuem escopo global.
            {
                f2 = 123 // Erro, pois já fo ra declarado antes.
...
Uma das maneiras de se fujir do objeto/escopo global é criando objetos:
    {
        ...
        /*
        - Acima criamos um atributo "falar", que recebe recebe uma função anônima capaz de retornar o nome de uma isntância atribuido ao atributo "nome" de uma suposta instância do objeto pessoa. Atenção ao "template-string" delimitado por crases, bem como a chamada de sua expressão (${}).
...
*/
    ...
    */
    ...
    Finalizando:
    Pesquisar:
    Runtime JavaScript (coisa mais importante);
    Padrões de projetos associados;
    Como escrever código;
    Como se pensar em JS;
    Paradigmas.

    E lembre-se, fuja do escopo global. Não coloque variáveis no escopo global, se precisar use constantes e se precisar tocar o escopo global, sempre toque ele a partir de um objeto, e dentro de tal objeto coloque suas coisas.
    */